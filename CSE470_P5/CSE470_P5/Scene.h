...
public:
	...
	SceneMaterial (void)
	{}
	static SceneMaterial* averageMaterial(SceneMaterial mat1, SceneMaterial mat2, SceneMaterial mat3, float s1, float s2, float s3){
		SceneMaterial *avg = new SceneMaterial();
		float total =s1+s2+s3;
		avg->diffuse=mat1.diffuse*(s1)+mat2.diffuse*(s2)+mat3.diffuse*(s3);
		avg->reflective=mat1.reflective*(s1)+mat2.reflective*(s2)+mat3.reflective*(s3);
		avg->refraction_index=mat1.refraction_index*(s1)+mat2.refraction_index*(s2)+mat3.refraction_index*(s3);
		avg->shininess=mat1.shininess*(s1)+mat2.shininess*(s2)+mat3.shininess*(s3);
		avg->specular=mat1.specular*(s1)+mat2.specular*(s2)+mat3.specular*(s3);
		avg->texture=mat1.texture;
		avg->transparent=mat1.transparent*(s1)+mat2.transparent*(s2)+mat3.transparent*(s3);
		return avg;
	}
	~SceneMaterial (void)
	{
