...
void menufunc(int value)
{
	...
	case 2:
		...
		break;
	case 3:
		if (!g_RayTrace.m_Scene.Load ("scene1.xml"))
		{
		...
		break;
	case 4:
		if (!g_RayTrace.m_Scene.Load ("scene2.xml"))
		{
			printf ("failed to load scene\n");
			exit(1);
		}
		...
		break;
	case 5:
		if (!g_RayTrace.m_Scene.Load ("scene3.xml"))
		{
			printf ("failed to load scene\n");
			exit(1);
		}
...
int main (int argc, char ** argv)
{
	...
	int loadmenu=glutCreateMenu(menufunc);
	...
	glutAddSubMenu("Load Scene",loadmenu);
